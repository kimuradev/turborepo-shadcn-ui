...
type PwaIOSModalProps = {
    show: boolean,
    onClose: () => void,
}

const ShareIcon = ({ className, modern = false }: any) => {
    if (modern) {
        return (
            <svg
...
const HomeScreenIcon = ({ className, modern = false }: any) => {
    if (modern) {
        return (
            <svg
                xmlns="http://www.w3.org/2000/svg"
...
const PwaIOSModal = ({ show, onClose }: PwaIOSModalProps) => {
    ...
    return (
        show && (
            <div className="fixed inset-0 flex items-end justify-center z-50 mb-2">
                <div ref={ref} className="bg-white w-94 p-4 rounded-lg shadow-lg">
                    <div className="flex items-center justify-center text-xs text-muted-foreground gap-2">
                        <HomeScreenIcon className="w-6 h-6" />

                        <div>
                            <p className="flex justify-center items-center gap-2">Instale o aplicativo clicando no ícone <ShareIcon className="w-4 h-4" /></p>
                            <p>e depois 'Adicionar a tela de início'</p>
                        </div>
                    </div>
                </div>
                <div
