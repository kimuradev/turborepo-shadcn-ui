...
import { CalendarIcon, ClockIcon, CalendarOff } from "lucide-react";
...
type DatePickerProps = {
    ...
    isDisabled?: boolean
    handleDatePickerDialog: (date: any) => void
...
const DateTimePicker = (props: DatePickerProps) => {
    ...
    useInteractOutside({
        ...
        onInteractOutside: async (e) => {
            if (props.value?.date && !props.hasSchedule) {
                await props.handleDatePickerDialog(props.value?.date);
            }
            setOpen(false);
        },
    });

    const handleRemoveDateTime = async () => {
        setIsLoading(true)
        try {
            await props.handleRemoveDate();
            setOpen(false);
        } finally {
            setIsLoading(false)
        }
                        ...
                        <div className="flex justify-center items-center p-3 text-primary hover:text-black ">
                            {!isLoading ? (
                                ...
                                >
                                    ...
                                    <span>Remover data do jogo</span>
                                </Button>
                            ) : <Spinner />}
                        </div>
                    )}
